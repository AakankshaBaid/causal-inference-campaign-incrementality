...
def plot_counterfactual(result, activation_week, outcome_label: str, path: Path) -> Path:
    """Actual versus modelled no-campaign path over the measurement window."""
    weekly = result.weekly
    fig, ax = plt.subplots(figsize=(8.0, 3.6))

    ax.fill_between(
        weekly["week"],
        weekly["counterfactual_low"],
        weekly["counterfactual_high"],
        color=COUNTERFACTUAL,
        alpha=0.28,
        linewidth=0,
        label=f"{int((1 - result.alpha) * 100)}% counterfactual interval",
    )
    ax.plot(
        weekly["week"],
        weekly["counterfactual"],
        color=COUNTERFACTUAL,
        linestyle="--",
        linewidth=1.8,
    ...
    ax.axvline(activation_week, color=INK, linewidth=1.0, linestyle=":")
    ax.annotate(
        "deal live",
        xy=(activation_week, ax.get_ylim()[1]),
        xytext=(4, -10),
    ...
    ax.set_title(f"Measured campaign window: actual vs. counterfactual ({outcome_label})")
...
def plot_uplift(result, path: Path) -> Path:
    """Weekly and cumulative effect, which is where fade shows up."""
    weekly = result.weekly
    ...
    axes[0].set_title("Weekly relative uplift")
    axes[0].yaxis.set_major_formatter(FuncFormatter(_pct))
    axes[0].grid(axis="y", linewidth=0.6)

    axes[1].plot(
        weekly["week"], weekly["cumulative_uplift"], color=ACCENT, linewidth=2.2, marker="o", markersize=3.5
    )
    axes[1].axhline(0, color=INK, linewidth=0.8)
    axes[1].fill_between(
        weekly["week"], 0, weekly["cumulative_uplift"], color=ACCENT, alpha=0.12, linewidth=0
    )
    axes[1].set_title("Cumulative incremental booking value")
...
def plot_validation(report, observed_relative: float, path: Path) -> Path:
    """The falsification battery on one page."""
    fig, axes = plt.subplots(1, 3, figsize=(11.0, 3.2))

    # -- panel 1: in-space placebo null ---------------------------------
    effects = report.placebo_distribution
    axes[0].hist(effects, bins=14, color=COUNTERFACTUAL, edgecolor="white", linewidth=0.7)
    axes[0].axvline(observed_relative, color=NEGATIVE, linewidth=2.0)
    axes[0].annotate(
        f"observed {observed_relative:+.1%}",
        xy=(observed_relative, axes[0].get_ylim()[1] * 0.88),
        xytext=(-6, 0),
        textcoords="offset points",
        ha="right",
        fontsize=8,
    ...
    axes[0].set_title(f"In-space placebo null (n={len(effects)})")
    axes[0].set_xlabel("estimated uplift on untreated cohorts")
    axes[0].xaxis.set_major_formatter(FuncFormatter(_pct))

    # -- panel 2: specification sensitivity tornado ----------------------
    sens = report.sensitivity.sort_values("relative_uplift")
    ypos = np.arange(len(sens))
    bar_colors = [
        ACCENT if ok else NEGATIVE for ok in sens.get("defensible", [True] * len(sens))
    ]
    axes[1].barh(ypos, sens["relative_uplift"], color=bar_colors, alpha=0.75, height=0.6)
    axes[1].axvline(observed_relative, color=INK, linestyle=":", linewidth=1.2)
    axes[1].set_yticks(ypos)
    axes[1].set_yticklabels(sens["variant"], fontsize=7.5)
    axes[1].set_title("Specification sensitivity")
    axes[1].text(
        0.02, -0.28, "red = estimator rejected by the recovery study",
        transform=axes[1].transAxes, fontsize=6.5, color=NEGATIVE,
    )
    axes[1].xaxis.set_major_formatter(FuncFormatter(_pct))
    axes[1].grid(axis="x", linewidth=0.6)

    # -- panel 3: scorecard ---------------------------------------------
    # Rows are laid out from a running cursor rather than a fixed pitch,
    # because detail strings vary from one line to four and a fixed pitch
    # overlaps them into illegibility.
    axes[2].axis("off")
    axes[2].set_title("Falsification scorecard", loc="left")

    wrapped = [
        (test, textwrap.wrap(test.detail, width=58)[:3]) for test in report.tests
    ]
    line_h = 0.042
    total = sum(1.6 + len(lines) for _, lines in wrapped)
    y = 0.97
    scale = min(1.0, 0.97 / (total * line_h))

    for test, lines in wrapped:
        mark = "PASS" if test.passed else "FAIL"
        color = POSITIVE if test.passed else NEGATIVE
        axes[2].text(0.0, y, test.name, fontsize=8.5, color=INK, va="top")
        axes[2].text(
            1.0, y, mark, fontsize=8.5, color=color, ha="right", va="top", weight="bold"
        )
        y -= line_h * 1.25 * scale
        for line in lines:
...
def plot_economics(econ_result, grid: pd.DataFrame, path: Path) -> Path:
    """Value waterfall plus the decision surface."""
    fig, axes = plt.subplots(1, 2, figsize=(10.5, 3.6))

    rows = econ_result.pnl_rows()
    labels = rows["line_item"].tolist()
    ...
    axes[0].set_title("Campaign value build (P&L)")
    ...
    extent = [
        grid["relative_uplift"].min(),
        grid["relative_uplift"].max(),
        grid["discount_depth"].min(),
        grid["discount_depth"].max(),
    ]
    ...
    axes[1].clabel(contour, fmt={0.0: "break-even"}, fontsize=7.5)
    axes[1].set_title("Net value by lift and discount depth")
    axes[1].set_xlabel("relative uplift")
    axes[1].set_ylabel("discount depth")
...
def plot_drivers(decomposition, path: Path) -> Path:
    ...
    ax.set_title("What drove the lift  —  green adds, red is given back")
...
def plot_assumptions(
    ...
    fig.suptitle(
        "Model assumptions", fontsize=13, fontweight="bold", x=0.075, ha="left", y=0.99
    )
    ...
    ax1.set_xlabel("Weekly volume per property ($000)")
    ax1.set_title("1  Cohorts are comparable", loc="left", fontsize=10.5)
    ...
    ax2.set_title(
        "2i  Predictors show no shift after the campaign starts", loc="left", fontsize=10.5
    )
    ax2.set_yticks([])
    ax2.legend(fontsize=6.4, ncol=3, loc="lower left")
    ...
    ax3.axis("off")
    ax3.set_title(
        "2ii  A period with no campaign reads as no campaign", loc="left", fontsize=10.5
    )
